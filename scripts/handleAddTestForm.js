...
const addTestForm = (testData) => {
  ...
  formColumn2.appendChild(
    createSearchInput("text", "กรอกชื่อรายการทดสอบ (ภาษาอังกฤษ)")
  );
  ...
  formColumn4.appendChild(
    createSearchInput("number", "กรอกระยะเวลาดำเนินงาน (วัน)")
  );
  ...
  const formColumn6 = createCheckboxColumn(
    "มาตรฐาน ISO 17025",
    "ได้การรับรองมาตรฐาน ISO 17025"
  );
  ...
  const formColumn9 = createTextAreaColumn(
    "สาเหตุการงดให้บริการ",
    "กรอกสาเหตุการงดให้บริการ"
  );
  ...
  if (testData) {
    ...
    formColumn2.querySelector(
      'input[placeholder="กรอกชื่อรายการทดสอบ"]'
    ).value = testName;
    formColumn2.querySelector(
      'input[placeholder="กรอกชื่อรายการทดสอบ (ภาษาอังกฤษ)"]'
    ).value = testNameEn;
    ...
    const option = Array.from(responsibleAgencySelect.options).find(
      (option) => option.textContent === responsibleAgency
    );
    if (option) option.selected = true;

    formColumn4.querySelector("input").value = price;
    formColumn4.querySelector(
      'input[placeholder="กรอกระยะเวลาดำเนินงาน (วัน)"]'
    ).value = executionTime;
    ...
    statusRadios.forEach((radio) => {
      if (
        radio.value === (status === "ให้บริการ" ? "available" : "not-available")
      ) {
        radio.checked = true;
      }
    });
...
const createRadioColumn = (labelText, options) => {
  ...
  options.forEach((option) => {
    ...
    radio.innerHTML = `
            <input type="radio" name="${labelText}" value="${option.value}" />
            <span></span>
            ${option.label}
        `;
