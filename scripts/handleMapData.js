...
export const mapProductTable = () => {
  ...
  products.forEach((product) => {
    const row = document.createElement("tr");
    const statusClass =
      product.serviceStatus === "ให้บริการ"
        ? "status-available"
        : "status-not-available";
...
const mapProductLstTable = (product) => {
  ...
  row.innerHTML = `
      ...
      <td>${product.responsibleAgency}</td>
      <td>${product.price}</td>
      <td>${product.executionTime} วัน</td>
      <td><span class="${statusClass}">${product.status}</span></td>
      <td>
        <div class="action-button">
          <button class="button primary-button"
            onclick="openModalProduct(); addTestForm(decodeURIComponent('${encodeURIComponent(
...
export const mapResponsibleAgencySelect = () => {
  ...
  const responsibleAgencySelect = document.getElementById(
    "responsibleAgencySelect"
  );
...
export const mapProductGroupForm = (productGroupId) => {
  const productGroup = PRODUCT_GROUPS.find(
    (group) => group.productGroupId === productGroupId
  );

  const idProductGroup = document.querySelectorAll(
    'span[class="id-product-group"]'
  );
  ...
  document.querySelector('input[placeholder="กรอกรหัสกลุ่มผลิตภัณฑ์"]').value =
    productGroup.productGroupId;
  document.querySelector('input[placeholder="กรอกชื่อกลุ่มผลิตภัณฑ์"]').value =
    productGroup.productGroupName;
  document.querySelector(
    'input[placeholder="กรอกชื่อกลุ่มผลิตภัณฑ์ (ภาษาอังกฤษ)"]'
  ).value = productGroup.productGroupNameEn;

  const productTypeRadio = document.querySelectorAll(
    'input[name="type-product-group"]'
  );
  productTypeRadio.forEach((radio) => {
    if (
      radio.value ===
      (productGroup.productType === "ทดสอบ" ? "testing" : "comparison")
    ) {
      radio.checked = true;
    }
  });
  ...
  const option = Array.from(responsibleAgencySelect.options).find(
    (option) => option.textContent === productGroup.responsibleAgency
  );
  option.selected = true;

  const serviceStatusRadio = document.querySelectorAll(
    'input[name="status-product-group"]'
  );
  serviceStatusRadio.forEach((radio) => {
    if (
      radio.value ===
      (productGroup.serviceStatus === "ให้บริการ"
        ? "available"
        : "not-available")
    ) {
      radio.checked = true;
  ...
  document.querySelector(
    'textarea[placeholder="กรอกสาเหตุการงดให้บริการ"]'
  ).value = productGroup.reason;
};
...
export const mapProductForm = (productCode) => {
  ...
  const productGroupSelect = document.getElementById("productGroupSelect");
  const option = Array.from(productGroupSelect.options).find(
    (option) => option.textContent === product.productGroup
  );
  option.selected = true;

  const productCodeInput = document.querySelector(
    'input[placeholder="กรอกรหัสผลิตภัณฑ์"]'
  );
  productCodeInput.value = product.productCode;

  const productNameInput = document.querySelector(
    'input[placeholder="กรอกชื่อผลิตภัณฑ์"]'
  );
  productNameInput.value = product.productName;

  const productNameEnInput = document.querySelector(
    'input[placeholder="กรอกชื่อผลิตภัณฑ์ (ภาษาอังกฤษ)"]'
  );
  productNameEnInput.value = product.productNameEn;

  const serviceStatusRadios = document.querySelectorAll(
    'input[name="status-product-group"]'
  );
  serviceStatusRadios.forEach((radio) => {
    if (
      radio.value ===
      (product.serviceStatus === "ให้บริการ" ? "available" : "not-available")
    ) {
      radio.checked = true;
    }
  });
  ...
  if (product.testItems.length) {
    product.testItems.forEach((testItem) => {
      mapProductLstTable(testItem);
...
export const mapFeeForm = (feeId) => {
  ...
  const improvementDetailTextarea = document.querySelector(
    'textarea[placeholder="กรอกรายละเอียดการปรับปรุง"]'
  );
